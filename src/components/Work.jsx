...
const Work = () => {
  return (
    <div className='flex justify-center items-center'>
        ...
        {/* Aonflow */}
        {<WorkTemplate profile={"Software Engineer"} org={"Aonflow (AX)"} Location={"Gurugram"} mode={"On-Site"} start={"Sept, 2024"} end={"Jan, 2026"}
        ...
        ></WorkTemplate>}
        {/* CRPTM */}
        {<WorkTemplate profile={"Backend Engineer"} org={"CRPTM (AX)"} Location={"Gurugram"} mode={"On-Site"} start={"Aug, 2025"} end={"Dec, 2025"}
...
<span>{Location}</span>
    ...
    <div className='block md:hidden lg:hidden  text-left ml-4 text-sm mx-2 font-mono font-semibold text-amber-400'>{start} - {end}</div>
    </div>
    <div className='hidden md:block text-right mr-5 font-mono text-lg font-semibold text-amber-400'>{start} - {end}</div>
    </div>
    <div className="mt-2 text-left lg:mt-5 px-4 font-mono font-medium text-sm md:text-xl text-slate-400 pb-3 tracking-wide md:tracking-wider">&bull;<span className="mr-2"/>{pointOne}<br/>&bull;<span className="mr-2"/>{pointTwo}<br/>&bull;<span className="mr-2"/>{pointThree}</div>
     </motion.div>
    </div>
  )
}
